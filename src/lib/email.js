...
function proposalDecisionTemplate({ clientName, clientEmail, status, packageName, price, projectTitle, reason }) {
  ...
  const body = accepted
    ? `<strong style="color:#ffffff;">${clientName}</strong> (${clientEmail}) وافق على عرض <strong style="color:#ffffff;">"${projectTitle}"</strong> واختار باقة <strong style="color:#ffffff;">${packageName}</strong> بقيمة <strong style="color:#ffffff;">${Number(price).toLocaleString("en-US")}</strong>. جاهز تبدأ تجهز المشروع والمراحل له من لوحة التحكم.`
    : `<strong style="color:#ffffff;">${clientName}</strong> (${clientEmail}) رفض عرض <strong style="color:#ffffff;">"${projectTitle}"</strong>.<br/><br/>السبب اللي كتبه: <strong style="color:#ffffff;">${reason}</strong>`;
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#1a1440;padding:40px 16px;font-family:-apple-system,'Segoe UI',Tahoma,Arial,sans-serif;" dir="rtl" lang="ar"><tr><td align="center"><table role="presentation" width="480" cellpadding="0" cellspacing="0" style="max-width:480px;width:100%;background-color:#28224e;border:1px solid rgba(239,75,122,0.35);border-radius:18px;overflow:hidden;"><tr><td style="padding:0;line-height:0;"><div style="height:5px;width:100%;background-color:${accent};"></div></td></tr><tr><td style="padding:28px 32px;text-align:center;"><h1 style="margin:0 0 16px 0;font-size:19px;color:#ffffff;">${heading}</h1><p style="margin:0;font-size:14px;line-height:1.9;color:#cfd2ea;">${body}</p></td></tr></table></td></tr></table>`;
}
