...
// Padding after the real preheader text so preview-snippet extraction (inbox
// list, lock-screen push) runs out of "room" inside the hidden div instead of
// spilling into the visible greeting/body that follows.
const PREHEADER_PADDING = "&nbsp;&zwnj;".repeat(80);
...
function stagePaymentTemplate({ clientName, projectTitle, stage, portalUrl }) {
  ...
  const preheader = `<div style="display:none;font-size:1px;line-height:1px;max-height:0;max-width:0;overflow:hidden;opacity:0;mso-hide:all;">${STAGE_PAYMENT_PREVIEW_TEXT}${PREHEADER_PADDING}</div>`;
...
function proposalDecisionTemplate({ clientName, clientEmail, status, packageName, price, projectTitle, reason }) {
  ...
  const infoRows = accepted
    ? `<tr><td style="padding:6px 0;font-size:13px;color:#a9adcf;">المشروع</td><td style="padding:6px 0;font-size:14px;color:#ffffff;font-weight:700;text-align:left;">${emailProjectTitle}</td></tr><tr><td style="padding:6px 0;font-size:13px;color:#a9adcf;">الباقة المختارة</td><td style="padding:6px 0;font-size:14px;color:#ffffff;font-weight:700;text-align:left;">${cleanPackageName}</td></tr><tr><td style="padding:6px 0;font-size:13px;color:#a9adcf;">القيمة</td><td style="padding:6px 0;font-size:16px;color:#ffffff;font-weight:900;text-align:left;">${Number(price).toLocaleString("en-US")} ريال</td></tr>`
...
export async function sendStagePaymentEmail({
  ...
  const { data, error } = await resend.emails.send({
    ...
    // Some mail apps build the lock-screen/notification preview from this
    // plain-text alternative rather than the HTML body — keep it identical
    // to the short phrase so nothing else ever leaks into the preview.
    text: STAGE_PAYMENT_PREVIEW_TEXT,
