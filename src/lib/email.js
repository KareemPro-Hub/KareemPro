...
export async function sendPaymentReceivedEmail({ to, clientName, projectTitle, stage }) {
  ...
  const { data, error } = await resend.emails.send({
    from: process.env.RESEND_FROM,
    to,
    subject: `✅ تم تأكيد استلام دفعة "${stage.title}"`,
